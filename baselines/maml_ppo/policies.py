...
class MlpPolicy(object):
    ...
    def __init__(self, ob_space, ac_space, nbatch, nsteps, sampling_pol=False): #pylint: disable=W0613
        ...
        if sampling_pol:
            ...
            self.weight_placeholders = {key: tf.placeholder(self.weights[key].get_shape()) for key in self.weights}
            self.assign_weight_ops = [tf.assign(weights[key], self.weight_placeholders[key]) for key in self.weights]
        else:
            self.weight_placeholders = None
            # stuff that is specific to making updated weights graph?
            pass

        self.pi, self.value, logstd = self.forward(X, self.all_weights)
    ...
    def construct_weights(self, scope):
        ...
        weights.update({'vw1': vw1, 'vb1': vb1, 'vw2': vw2, 'vb2': vb2, 'vw3': vw3, 'vb3': vb3})
        return weights
